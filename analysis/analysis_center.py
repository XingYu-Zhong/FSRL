...
class AnalysisCenter:
    ...
    def compare_strategy(self, country, start_time, end_time, code_list, init_balance, task_name):
        data_time = (datetime.datetime.strptime(start_time, '%Y%m%d') - datetime.timedelta(days=60)).strftime('%Y%m%d')
        get_data = GetData(country=country, start_date=data_time, end_date=end_time, code_list=code_list)
        ...
        vwap_strategy_copy = copy.deepcopy(out_strategy)  # 添加VWAP策略的副本
        ...
        rsi_strategy_df = rsi_strategy_copy.rsi_strategy(cash=init_balance, start_date=train_start_time, end_date=train_end_time, code_list=code_list)
        ...
        true_empty_strategy_df = true_empty_strategy_copy.true_empty_strategy(cash=init_balance, start_date=train_start_time, end_date=train_end_time, code_list=code_list)
        vwap_strategy_df = vwap_strategy_copy.vwap_strategy(cash=init_balance, start_date=train_start_time, end_date=train_end_time, code_list=code_list)  # 运行VWAP策略

        
        vwap_strategy_df.to_csv(os.path.join(root_path, f"{task_name}_vwap_strategy_df_{start_time}_to_{end_time}.csv"))
        ...
        sharpe_ratio_no_risk, sharpe_ratio, max_drawdown, annualized_return, annualized_volatility, total_return, total_profits = Evaluation(
            sr_risk=0.02).standard_evaluation(df=vwap_strategy_df)

        logger.info(f"strategy:vwap_strategy_df")
        ...
        vwap_strategy_result_pd = pd.DataFrame(data=result_list, columns=['vwap_strategy'],
        ...
        all_result_pd = pd.concat([orgin_result_pd,vwap_strategy_result_pd, twoma_strategy_result_pd,bulin_strategy_result_pd,rsi_strategy_result_pd,empty_strategy_result_pd,true_empty_strategy_result_pd])
        print(all_result_pd)
