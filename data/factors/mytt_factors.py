...
TAQ：计算股票的唐安奇通道指标，包括UP、MID和DOWN三个指标。
KTN：计算股票的肯特纳通道指标，包括UPPER、MID和LOWER三个指标。
TRIX：计算股票的TRIX指标，包括TRIX和TRMA两个指标。
VR：计算股票的VR指标。
EMV：计算股票的EMV指标，包括EMV和MAEMV两个指标。
DPO：计算股票的DPO指标，包括DPO和MADPO两个指标。
BRAR：计算股票的BRAR指标，包括AR和BR两个指标。
DFMA：计算股票的平行线差指标，包括DIF和DIFMA两个指标。
MTM：计算股票的动量指标，包括MTM和MTMMA两个指标。
MASS：计算股票的梅斯线指标，包括MASS和MA_MASS两个指标。
ROC：计算股票的ROC指标，包括ROC和MAROC两个指标。
EXPMA：计算股票的EMA指数平均数指标，包括N1和N2两个指标。
OBV：计算股票的能量潮指标。
MFI：计算股票的MFI指标。
ASI：计算股票的振动升降指标，包括ASI和ASIT两个指标。
XSII：计算薛斯通道II
"""

def MACD(CLOSE, SHORT=12, LONG=26, M=9):  # EMA的关系，S取120日，和雪球小数点2位相同
    """
    在MACD函数中，共有四个参数，分别是CLOSE、SHORT、LONG和M。

CLOSE：这是一个必需参数，表示收盘价的数据序列。

SHORT：这是可选参数，默认值为12，表示短期EMA的计算周期。

LONG：这是可选参数，默认值为26，表示长期EMA的计算周期。

M：这是可选参数，默认值为9，表示计算DEA的EMA的周期。

这些参数用于计算MACD指标，具体地，使用收盘价序列CLOSE计算出短期EMA和长期EMA，
然后计算它们的差值DIF，再用DIF计算出DEA的EMA，最后用DIF和DEA的差值的两倍计算出MACD指标。
这些参数的不同取值会影响指标的计算结果和对市场的解读。
例如，较小的SHORT和LONG值将导致MACD更加敏感，而较大的值则会使MACD更加平滑。
    """
...
HIGH：最高价序列。
LOW：最低价序列。
N：计算RSV值时取N日的最高价和最低价。默认值为9。
M1：计算K值时的平滑系数。默认值为3。
M2：计算D值时的平滑系数。默认值为3。
其中，RSV值是KDJ指标的核心计算步骤之一，表示当前收盘价在过去N日内最高价和最低价的比例关系，
通常取值范围在0到100之间。K值是RSV值的M1日指数移动平均线，用来反映最近几天内股价的强弱程度，
通常在0到100之间波动。D值是K值的M2日指数移动平均线，用来反映最近几天内股价的趋势变化，
通常在0到100之间波动。J值是K值和D值的加权平均值，用来综合反映股价的强弱程度和趋势变化，通常在0到100之间波动。
    :return:
    """
